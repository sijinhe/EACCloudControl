...
public class StartAPP extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (aact.notifyStart(container)) {

                container = aact.startAPP(container);

                if (container != null) {
                    json = jc.createJSON(container, true, "Start APP [DB Insertion Success]");
                } else {
                    json = jc.createJSON(container, false, "Start APP [DB Insertion Failed]");
                }

            } else {
                json = jc.createJSON(container, false, "Start APP [Start APP Failed]");
    ...
    private String startAPP(String containerid) {
        Container container = new Container(containerid);
